import { Popover, Grid, GridItem, PopoverTrigger, PopoverContent, PopoverArrow, PopoverCloseButton, PopoverHeader, PopoverBody, HStack, IconButton, Text, useDisclosure, VStack, Slide, ButtonGroup, Box, Button } from "@chakra-ui/react";
import React from "react";
import { FaBars, FaArrowDown, FaRegHandPointer } from "react-icons/fa";
import { IoClose, IoPersonOutline, IoGridOutline, IoHomeOutline, IoDocumentTextOutline, IoExtensionPuzzleOutline, IoBookOutline, IoFlaskOutline } from "react-icons/io5";
import { BsArrowBarDown } from "react-icons/bs";
import { useColourContext } from "../Contexts/ColourContext";
import { useDataContex } from "../Contexts/DataContext";
import AdminAuth from "./AdminAuth";

const navList = ["Home", "About", "Skills", "Experience", "Education", "Projects"];
const navIcon = [<IoHomeOutline />, <IoPersonOutline />, <IoDocumentTextOutline />, <IoExtensionPuzzleOutline />, <IoBookOutline />, <IoFlaskOutline />];
const themes = ["64,113,244", "246,99,72", "127,112,49", "112,95,220", "61,67,83", "206,121,115", "155,124,105"];
function Slider({ isOpen, onToggle }) {
  ...
  return (
    <Slide direction="bottom" in={isOpen} style={{ zIndex: 10 }}>
      <Box bg={"white"}>
        <Box style={{ background: "rgba(" + themeColor + ", 0.1)", transition: "ease-in 0.2s" }} h={"500px"} p={"4"} mr={"2"}>
          <HStack my={"4"} p={"4"} borderRadius="full" justifyContent={"space-around"} border={"1px solid rgb(" + themeColor + ")"}>
            {themes.map((theme, i) => (
